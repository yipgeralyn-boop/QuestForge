// map.jsx — QuestForge illustrated adventure map
// Exports to window: AdventureMap
// Renders an SVG parchment map (land/water/roads/decor) + a winding dashed
// route through the stops + numbered pennant pins + a "you are here" avatar.

(function () {
  const { useMemo } = React;

  // Smooth cardinal-spline path through a list of {x,y} points
  ...
  // Decorative static layer — drawn in a 0..100 coordinate space
  ...
  function AdventureMap({
    ...
    selectedId = null, onPinTap, height = 300, animateRoute = true,
    placing = false, onPlace, ghost = null,
  }) {
    const ms = (window.MAP_STYLES[mapStyle] || window.MAP_STYLES.parchment);
    const pts = stops.map(s => ({ x: s.x, y: s.y }));

    const fullD = useMemo(() => smoothPath(pts), [JSON.stringify(pts)]);
    const doneD = useMemo(() => {
      if (mode !== 'play') return '';
      const slice = pts.slice(0, Math.max(1, currentIndex + 1)).slice(0, currentIndex < 0 ? 0 : currentIndex + 1);
    ...
    return (
      <div style={{
        ...
        {/* route overlay (same coordinate space as decor, but route only) */}
        <svg viewBox="0 0 100 100" preserveAspectRatio="xMidYMid slice"
        ...
        {/* Pins as HTML for crisp theming */}
        {stops.map((s, i) => {
        ...
        {/* ghost preview pin while placing */}
        {ghost && (
          <div style={{
